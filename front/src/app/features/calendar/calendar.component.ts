...
export class CalendarComponent {
  ...
  /**
   * Get sessions that should be rendered in a specific hour slot
   * Only returns sessions that START in this hour slot to avoid duplicates
   * Multi-slot sessions will be rendered once with extended height
   */
  getSessionDataForDateAndHour(date: Date, hour: string): SessionData[] {
    const sessions = this.getSessionDataForDate(date);

    // Parse the hour slot (e.g., "13:00")
    ...
    return sessions.filter((data) => {
      const startTime = data.SessionDetailData.start_time.substring(0, 5);
      const [startHour, startMinute] = startTime.split(':').map(Number);
      const sessionStartMinutes = startHour * 60 + startMinute;

      // Only render sessions that START within this slot
      // This prevents rendering the same session multiple times
      return sessionStartMinutes >= slotStartMinutes && sessionStartMinutes < slotEndMinutes;
    });
  }

  /**
   * Get all sessions that overlap with a specific slot (for collision detection)
   */
  private getAllSessionsOverlappingSlot(date: Date, hour: string): SessionData[] {
    const sessions = this.getSessionDataForDate(date);
    const [slotHour, slotMinute] = hour.split(':').map(Number);
    const slotStartMinutes = slotHour * 60 + slotMinute;
    ...
    return sessions.filter((data) => {
      const startTime = data.SessionDetailData.start_time.substring(0, 5);
      const endTime = data.SessionDetailData.end_time.substring(0, 5);
  ...
  /**
   * Calculate the visual properties for rendering a session
   * Sessions are rendered as continuous cards that can span multiple hour slots
   * Uses a column-based layout (Google Calendar style) for overlapping sessions
   * Returns:
   * - heightPx: Absolute height in pixels based on session duration
   * - topOffsetPercentage: Offset from top of the starting slot (0-100)
   * - leftOffsetPercentage: Horizontal offset for column positioning (0-100)
   * - widthPercentage: Width of the column (0-100)
   */
  getSessionSlotRenderInfo(sessionData: SessionData, slotHour: string, date: Date): {
    heightPx: number;
    topOffsetPercentage: number;
    leftOffsetPercentage: number;
    widthPercentage: number;
  } {
    const [slotHourNum, slotMinuteNum] = slotHour.split(':').map(Number);
    const slotStartMinutes = slotHourNum * 60 + slotMinuteNum;

    const startTime = sessionData.SessionDetailData.start_time.substring(0, 5);
    const endTime = sessionData.SessionDetailData.end_time.substring(0, 5);
    ...
    const sessionStartMinutes = startHour * 60 + startMinute;
    const sessionEndMinutes = endHour * 60 + endMinute;
    const sessionDurationMinutes = sessionEndMinutes - sessionStartMinutes;

    // Calculate vertical positioning
    // Top offset: distance from slot start to session start
    const topOffsetPercentage = ((sessionStartMinutes - slotStartMinutes) / 60) * 100;

    // Height: total duration of the session in pixels
    // Assuming each hour slot is 64px (h-16) on mobile and 80px (h-20) on desktop
    // We'll use a base of 80px per hour for calculation
    const pixelsPerMinute = 80 / 60; // 80px per hour = ~1.33px per minute
    const heightPx = sessionDurationMinutes * pixelsPerMinute;

    // Calculate column-based layout for overlapping sessions
    // We need to check for overlaps across ALL slots the session might occupy
    const allOverlappingSessions = this.findOverlappingSessions(sessionData, date);
    const sessionIndex = allOverlappingSessions.findIndex(
      s => s.SessionDetailData.session_id === sessionData.SessionDetailData.session_id
    );
    const totalOverlapping = allOverlappingSessions.length;

    // Column layout: divide the available width into equal columns
    let leftOffsetPercentage = 0;
    let widthPercentage = 100;

    if (totalOverlapping > 1) {
      widthPercentage = 100 / totalOverlapping;
      leftOffsetPercentage = sessionIndex * widthPercentage;
    }
  ...
  /**
   * Find all sessions that overlap with the given session's time range
   */
  private findOverlappingSessions(sessionData: SessionData, date: Date): SessionData[] {
    const allSessions = this.getSessionDataForDate(date);
    ...
    // Find all sessions that overlap with this session's time range
    const overlapping = allSessions.filter((otherSession) => {
      const otherStart = otherSession.SessionDetailData.start_time.substring(0, 5);
      const otherEnd = otherSession.SessionDetailData.end_time.substring(0, 5);
      const [otherStartHour, otherStartMinute] = otherStart.split(':').map(Number);
      const [otherEndHour, otherEndMinute] = otherEnd.split(':').map(Number);
    ...
    // Sort by start time to maintain consistent column ordering
    return this.getSortedSessionDataForDate(date).filter(s =>
      overlapping.some(o => o.SessionDetailData.session_id === s.SessionDetailData.session_id)
    );
  }
